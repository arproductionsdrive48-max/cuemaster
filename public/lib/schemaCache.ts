...
export const warmUpSchemaCache = async (): Promise<void> => {
  ...
  try {
    ...
    console.log('[CueMaster] Schema cache warmed up for', SCHEMA_TABLES.length, 'tables');
  } catch (e) {
    console.warn('[CueMaster] Schema warm-up failed (non-critical):', e);
  }
};
...
export const withSchemaRetry = async <T>(fn: () => Promise<T>): Promise<T> => {
  try {
    ...
    if (isSchemaErr) {
      console.warn('[CueMaster] PGRST204 – refreshing schema cache and retrying in 1s…', msg);
      await warmUpSchemaCache();
